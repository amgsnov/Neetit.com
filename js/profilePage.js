...
function sendPost(event) {
  ...
  postGeneration(newPost.title, newPost.text, newPost.id);
...
function deletePost(event) {
  // delete button check
  const parentNode = event.target.closest(".block-post");
  const id = event.target.getAttribute("data-id");
  posts = posts.filter((post) => post.id != id);
  // deleting a post from the markup
  saveLocal();
  parentNode.remove();
}

// function generation posts
function postGeneration(title, description, id) {
  // Create block div with class block-post
  ...
  <div class="block-post__content">
      ...
      <p class="block-post__subtitle">${description}</p>
      <div class="block-post__comment">
          <a href="./leaveComment.html?${id}">
...
function renderPost() {
  posts.forEach((element) => {
    postGeneration(element.title, element.text, element.id);
  });
}
