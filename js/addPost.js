function handlePostGeneration(event) {
  event.preventDefault(event);
  let title = event.target[0].value ?? "default";
  let description = event.target[1].value ?? "default";
  postGeneration(title, description);
}

let form = document.getElementById("postsForm");
form.addEventListener("submit", handlePostGeneration);

// function generation posts
function postGeneration(title, description) {
