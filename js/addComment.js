...
let commentData = JSON.parse(localStorage.getItem("postData")) ?? {};

function saveLocal() {
  localStorage.setItem("comments", JSON.stringify(comments));
}

function getNeededPost() {
...
if (localStorage.getItem("posts")) {
  ...
  <div class="block-post__content">
      <h1 class="block-post__title">${neededPost.title}</h1>
      <p class="block-post__subtitle">${neededPost.text}</p>
...
function sendComments(event) {
  ...
  const newComment = {
    ...
    postId: getNeededPost().id,
  };
  commentGeneration(newComment.text, newComment.id);
...
function deleteComment(event) {
  const parentNode = event.target.closest(".comment-author");
...
function commentGeneration(title, id) {
...
function renderComments() {
  getNeededComments().forEach((element) => {
    commentGeneration(element.text, element.id);
  });
...
form.addEventListener("submit", sendComments);
form.addEventListener("input", saveInputsData);
renderComments();
